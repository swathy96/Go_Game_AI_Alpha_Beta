...
class Board:
    ...
    def find_neighbors(self, row, column):
        ...
        if column > 0: neighbors.add((column, column-1))
        if column < self.size - 1: neighbors.add((row, column+1))
        return neighbors

    def find_like_friends(self, row, column,board_state):
        neighbors = self.find_neighbors(row, column)  
        neighbor_friends = set()
        for neighbor in neighbors:
            if board_state[neighbor[0]][neighbor[1]] != 0  and board_state[neighbor[0]][neighbor[1]] == board_state[row][column]:
                neighbor_friends.add(neighbor)
        return neighbor_friends

    def find_board_friends(self, row, column,board_state):
        stack = [(row, column)]  
        friends = [] 
        while len(stack) > 0:
            stone_position = stack.pop()
            friends.append(stone_position)
            neighbors = self.find_like_friends(stone_position[0], stone_position[1],board_state)
            for neighbor in neighbors:
                if neighbor not in stack and neighbor not in friends and neighbor:
                    stack.append(neighbor)
        return friends

    def find_liberty(self, row, column,board_state):
        board = self.board
        ally_members = self.find_board_friends(row, column,board_state)
        for member in ally_members:
            neighbors = self.find_neighbors(member[0], member[1])
            for neighbor in neighbors:
        ...
        for row in range(0,self.size):
            for column in range(0,self.size):
                if board_state[row][column] == player:
                    if self.find_liberty(row, column, board_state) == False:
    ...
    def calculate_current_liberty(self, move,board_state):
        neighbors = self.find_neighbors(move[0], move[1])
        liberty_count = 0
        for neighbor in neighbors:
        ...
        for row in range(0, self.size):
            for column in range(0,self.size):
                if self.is_move_valid(board_state,player,row,column):
                    if self.calculate_current_liberty((row,column),board_state) > 0:                   
    ...
    def find_near_opponents(self, player,board_state,move):
        board_friends = self.find_board_friends(move[0], move[1],board_state)
        player_friends = set()
        for friend in board_friends:
            neighbors = self.find_neighbors(friend[0], friend[1])
            for neighbor in neighbors:
...
class AlphaBetaAgent_A:
    ...
    def calculate_player_max_score(self,player,board_state,go_board,move):
        ...
        current_liberty = go_board.calculate_current_liberty(move,board_state)
        is_edge_move = go_board.is_edge_move(move)
        near_opponents_count = go_board.find_near_opponents(player,board_state,move)
        if is_edge_move:
            diff+=-1
        #if current_liberty == 0:
            #liberty_count = 0
        if player == white_player:
            diff+=2.5
        return diff+liberty_count+len(captured_stones)+current_liberty
    ...
    def calculate_player_liberty(self,board_state,go_board,player,black_positions,white_positions):
        ...
        for black in black_positions:
            board_friends = go_board.find_board_friends(black[0], black[1],board_state)
            for friend in board_friends:
                neighbors = go_board.find_neighbors(friend[0], friend[1])
                for neighbor in neighbors:
                    if board_state[neighbor[0]][neighbor[1]] == 0:
                        black_liberty_count+=1 # Edit here okay
        for white in white_positions:
            board_friends = go_board.find_board_friends(white[0], white[1],board_state)
            for friend in board_friends:
                neighbors = go_board.find_neighbors(friend[0], friend[1])
                for neighbor in neighbors:
                    if board_state[neighbor[0]][neighbor[1]] == 0:
                        white_liberty_count+=1 # Edit here okay
    ...
    def get_input(self,go_game,player):
        ...
        play_move_details = self.alpha_beta_pruning(2,-5000,5000,True,None,go_board,board_state,player)
...
def main():
    ...
    player_move = AlphaBetaAgent_A()
    player_move_value = player_move.alpha_beta_pruning(2,-5000,5000,True,None,go_board,current_board_state,player)
